 ...
 * On UNIQUE constraint collision (race condition), retries up to MAX_RETRIES times by
 * incrementing the sequence number.
 *
 * Note: The caller is responsible for the actual DB insert with the returned serial.
 * If a UNIQUE violation occurs on insert, call this function again to get the next candidate.
 *
...
export async function generateCompetitionSerial(
  ...
  // Fallback: should never reach here (loop always returns)
